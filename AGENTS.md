...
- `frontend/` is a React 19 + TypeScript + Vite application with its own npm lockfile and commands. It has no CSS framework, no component library, no state manager, and no data-fetching library; do not add one without an explicit team decision. `src/styles/tokens.css` is the single source of colour, type, space, radius, elevation, z-index, breakpoint, and motion, and components consume semantic roles rather than literals, so a hex value in a component file is a defect. Styling is CSS Modules; fonts are self-hosted under `src/assets/fonts` precisely so a self-hosted deployment makes no third-party request. `src/ui/` holds the primitives and knows nothing about features, `src/app/` holds the shell and routing chrome, `src/features/<domain>/` holds screens, and `src/components/` keeps only the route guard, document rows, and credit chips. There is no page-level global CSS: every styled component owns a `Name.module.css` beside it, and `src/styles/tokenUsage.test.ts` fails the run if a module reads a custom property nothing defines, because an undefined `var()` fails silently. `Dialog` is the only modal and `ConfirmDialog` the only destructive confirmation: there is no `window.confirm`, no bespoke backdrop, and no second focus-trap implementation. `IconButton` requires `label` so an icon-only control cannot compile without an accessible name, the shell owns the single `<main>` so a page must not render its own, and no two controls inside one dialog may answer to the same name, which is why a footer never says Close next to the dismiss control that already does. A disabled control explains itself in visible text rather than a `title`, since a disabled button is neither focusable nor announced. Branch AI failures on the `X-Error-Code` header rather than the status, keep `retrieval_narrowed` and `context_truncated` distinct, read the credit balance only from `GET /api/users/me/credits`, treat a null balance as unmetered by rendering no credit UI at all, and never hardcode a generation price. No screen may show a number the backend cannot produce: a metric that is unavailable is absent, not zero. A quiz attempt reports `graded_count` beside `total_questions`; score is correct out of graded, and an ungraded written answer is counted separately and never as wrong. `setupTests.ts` turns React's duplicate-key, DOM-nesting, `act`, and ARIA warnings into failures, so a green run means the render was clean, and a regression test is only finished once you have watched it fail with the fix reverted. `npm run lint` passing does not mean `npm run build` passes, because the build type-checks tests too. See `docs/frontend_system.md`.
