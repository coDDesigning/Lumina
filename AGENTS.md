...
- `IMPLEMENTED_AI_PROVIDERS` in `backend/app/config.py` is the authoritative provider list, and a test asserts `get_text_generation_provider` constructs every name in it. `openai` and `claude` are recognized but unimplemented and fail at startup, never at request time. Providers return plain text or a JSON object; feature services own prompt construction, schema validation, and persistence. See `docs/ai_providers.md`.
