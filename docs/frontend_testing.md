...
| [`src/App.progress.test.tsx`](../frontend/src/App.progress.test.tsx) | Course cards populated from one `GET /api/progress` request rather than a per-course fan-out, and the unavailable fallback when that request fails while the course list still renders. |
| [`src/features/courses/CoursesPage.test.tsx`](../frontend/src/features/courses/CoursesPage.test.tsx) | Exam countdown and ordering, create and delete flows, and the three progress states — scored, no activity, and unavailable — plus the derived status badge and the last-studied line. |
