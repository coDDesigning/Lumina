...
| [`src/App.progress.test.tsx`](../frontend/src/App.progress.test.tsx) | Course cards populated from one `GET /api/progress` request rather than a per-course fan-out, the unavailable fallback when that request fails while the course list still renders, the status badge rendered from the value the backend sent rather than a client derivation, and time spent shown only for the courses that recorded it. |
| [`src/features/courses/CoursesPage.test.tsx`](../frontend/src/features/courses/CoursesPage.test.tsx) | Exam countdown and ordering, create and delete flows, and the three progress states — scored, no activity, and unavailable — plus the five course statuses, the line each one puts beneath the badge, and the last-studied and time-spent lines. |
| [`src/features/workspace/ProgressView.test.tsx`](../frontend/src/features/workspace/ProgressView.test.tsx) | Course progress stats, topic mastery, practice from a weak topic, attempt-history links back into a stored review, and the per-attempt and whole-course durations, which are absent rather than zero when nothing recorded them. |
| [`src/lib/formatStudyTime.test.ts`](../frontend/src/lib/formatStudyTime.test.ts) | Seconds, minutes and hour-plus spans, and the null that keeps a zero or nonsense duration off the screen. |
