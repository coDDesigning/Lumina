...
| [Stranded Tombstone Purge](stranded_tombstone.md) | Deleted accounts (`deletion_requested_at IS NOT NULL`), courses (`is_deleted = True`), and documents (`status = 'deleting'`) whose physical storage or vectors failed to delete cleanly. | `python -m workers.course_purge` |
...
| [System Settings & Controlled Restart](system_settings_restart.md) | Administrator configuration overrides that will not start, a restart stuck draining, and break-glass recovery of the override store. | `docker compose logs`, `/data/system-settings/`, `services.system_restart` |
| [Oracle Single-Host Production](oracle-single-host.md) | Current production on one Oracle Cloud VM: deploy pipeline, backups, restore, and rollback. | `ops/oracle/deploy.sh`, `ops/oracle/backup.sh`, `docker compose` |
...
| [SCRUM-208 Release Checklist](release_checklist_scrum_208.md) | Verification and release gate checklist for privacy-safe account hard delete and cleanup. | `workers.course_purge`, `alembic` |
