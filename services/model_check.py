...
MODEL_CHECK_TIMEOUT_SECONDS = 30.0
OLLAMA_TAGS_TIMEOUT_SECONDS = 5.0
MODEL_CHECK_TEMPLATE_NAME = "model_check"
_TAGS_PATH = "/api/tags"
...
_SUCCESS_MESSAGE = "The model responded successfully."
_UNREACHABLE_MESSAGE = "The model provider could not be reached. Try again in a moment."
_TIMEOUT_MESSAGE = "The model did not respond in time. Try again."
_AUTH_MESSAGE = (
...
def _perform_check(
    db: Session, *, user: User, is_admin: bool, model_id: str | None
    ...
    try:
        ...
        provider = with_template_temperature(
            provider, PromptLoader.temperature_for(MODEL_CHECK_TEMPLATE_NAME)
        )
        prompt_context = resolve_prompt_context(db, user_id=user.id)
        prompt = PromptLoader.render(
            MODEL_CHECK_TEMPLATE_NAME, prompt_context.as_variables()
        )
    ...
    if bool(entry.get("vision")) and provider_name == AI_PROVIDER_OLLAMA:
...
def check_model(
    ...
    result, log_provider, log_model = _perform_check(
        db, user=user, is_admin=is_admin, model_id=model_id
    )
