...
def test_request_password_reset_creates_token(api_context, monkeypatch):
    ...
    response = api_context.client.post("/api/auth/reset-password", json={"email": "reset@example.com"})
    ...
    with api_context.session_factory() as session:
        user = session.execute(select(User).where(User.email == "reset@example.com")).scalar_one()
        ...
        confirm_response = api_context.client.post("/api/auth/reset-password/confirm", json={
